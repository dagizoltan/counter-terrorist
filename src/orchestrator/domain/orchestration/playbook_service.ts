...
export class PlaybookService extends BaseService {
  ...
  protected override onInit(): Promise<Result<void>> {
    ...
    this.unsubscribers.push(eventBus.on("HONEYPOT", async (payloadObj: unknown) => { const payload = payloadObj as { type: string, port: number | string, source_ip: string };
    ...
    this.unsubscribers.push(eventBus.on("DRIFT_PROCESS", async (payloadObj: unknown) => { const payload = payloadObj as { path: string, action: string };
    ...
    this.unsubscribers.push(eventBus.on("EBPF_CRITICAL", async (payloadObj: unknown) => { const payload = payloadObj as { comm: string, syscall: string, pid?: number };
    ...
    this.unsubscribers.push(eventBus.on("THREAT", (payloadObj: unknown) => { const payload = payloadObj as { nodeId?: string, severity: string, path?: string };
    ...
    this.unsubscribers.push(eventBus.on("ARTIFACT_FOUND", async (payloadObj: unknown) => { const payload = payloadObj as { indicator: string, [key: string]: unknown };
       if (!payload.indicator) return;
       await this.executeArtifactContainment(payload.indicator, payload);
    }));

    // Cross-Platform Playbook Hooks
    this.unsubscribers.push(eventBus.on("ES_EXEC", (payloadObj: unknown) => { const payload = payloadObj as { path: string, signing_id?: string };
    ...
    this.unsubscribers.push(eventBus.on("ETW_PROCESS", (payloadObj: unknown) => { const payload = payloadObj as { command_line: string, process_name?: string };
