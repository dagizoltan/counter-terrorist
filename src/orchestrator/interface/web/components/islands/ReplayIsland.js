...
function ReplayIsland() {
  ...
  if (filteredEvents.length === 0) return html`
    <div class="empty-state">
       <span class="eyebrow">No_Events_Match_Filter_Criteria</span>
       <button onClick=${() => setFilter('ALL')} class="t-btn mt-4">Reset_Filter</button>
    </div>
  `;
  ...
  return html`
    <div class="space-y-4">
      <div class="t-panel glass-panel p-5">
        ...
        <div class="relative h-2 bg-white/5 rounded-full mb-4 group overflow-visible">
        ...
        <div class="eyebrow flex justify-between">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
         <div class="lg:col-span-8 space-y-4">
            <div class="t-panel glass-panel p-5 relative overflow-hidden border-l-4" style=${{ borderLeftColor: color }}>
               <div class="absolute top-0 right-0 p-5 opacity-[0.05] pointer-events-none transform rotate-12"><svg width="200" height="200" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div>
               <div class="flex justify-between items-start mb-5"><div class=${`status-pill ${theme}`}>${severity}</div><div class="flex flex-col items-end"><span class="metric-tag mb-1">EVENT_ID</span><span class="mono-xs font-black text-white bg-white/5 px-3 py-1 rounded">CT-${currentEvent.id?.toString().slice(-6) || 'UNK'}</span></div></div>
               <h4 class="text-2xl font-bold text-white mb-5 leading-tight tracking-tighter uppercase">${currentEvent.message}</h4>
               <div class="bg-black/60 rounded-lg border border-white/5 p-4"><div class="flex items-center justify-between mb-4 pb-4 border-b border-white/5"><div class="flex items-center gap-4"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--primary)" stroke-width="3"><polyline points="4 7 4 4 20 4 20 7"/><line x1="9" y1="20" x2="15" y2="20"/><line x1="12" y1="4" x2="12" y2="20"/></svg><span class="eyebrow" data-tone="primary">Telemetry_Manifest</span></div><span class="mono-xs text-slate-700">SHA256_VERIFIED</span></div><pre class="mono-xs text-slate-400 leading-relaxed overflow-x-auto custom-scrollbar max-h-[400px]">${JSON.stringify(currentEvent.data || {}, null, 2)}</pre></div>
            </div>
            <div class=${`t-panel border-l-4 p-8 ${theme === 'danger' ? 'border-danger bg-danger/5' : 'border-primary bg-primary/5'}`}>
               <div class="flex items-center justify-between mb-4"><div class="flex items-center gap-4 text-white"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg><span class="tactical-title text-base tracking-widest">BLOCKCHAIN_LEDGER_INTEGRITY</span></div><div class="status-pill active bg-success/20 border-success/30">VALIDATED</div></div>
               <div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div class="space-y-3"><div class="metric-tag uppercase">Previous_Consensus_Hash</div><div class="mono-xs text-slate-500 truncate bg-black/40 p-4 rounded border border-white/5 font-bold">${currentEvent.prevHash || '00000000000000000000000000000000'}</div></div><div class="space-y-3"><div class="metric-tag uppercase">Block_Certificate</div><div class="mono-xs text-success font-black truncate bg-black/40 p-4 rounded border border-success/10 tracking-widest">${currentEvent.hash}</div></div></div>
            </div>
         </div>
            ...
            <div class="t-panel glass-panel p-4">
               ...
               <div class="space-y-4">
                  ${['SOVEREIGN_A', 'SOVEREIGN_B', 'MESH_GATEWAY'].map((node, idx) => {
                    ...
                    return html`<div class=${`flex items-center justify-between p-5 bg-black/40 rounded border ${isActive ? 'border-primary/40' : 'border-white/5'}`}><div class="flex items-center gap-3"><span class="indicator" data-state=${isActive ? 'ok' : 'idle'} /><span class="mono-sm text-white">${node}</span></div><span class="eyebrow" data-tone=${isActive ? 'primary' : undefined}>${isActive ? 'Signal' : 'Idle'}</span></div>`;
                  })}
               </div>
               <div class="mt-5 p-4 bg-primary/5 border border-primary/10 rounded-lg relative overflow-hidden group">
                  <div class="absolute inset-0 bg-primary/5 translate-y-full"></div>
                  <div class="relative z-10"><div class="flex items-center gap-4 text-primary mb-5"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span class="tactical-title text-xs tracking-widest uppercase">Forensic_AI_Insight</span></div><p class="mono-xs text-slate-400 leading-relaxed font-bold tracking-tight">${theme === 'danger' ? '"Critical pattern identified. System suggests immediate review of adjacent temporal blocks for lateral movement signatures."' : '"Baseline integrity within operational norms. No anomalous drift detected in this sequence."'}</p></div>
               </div>
            </div>
            <div class="grid grid-cols-1 gap-4">
               <button onClick=${handleExport} class="t-btn w-full justify-center p-3 text-xs font-black border-2 group"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> GENERATE_EVIDENCE_BUNDLE</button>
               <button onClick=${handleIsolate} class="t-btn danger w-full justify-center p-3 text-xs font-black border-2 group"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> INITIATE_ISOLATION</button>
            </div>
         </div>
      </div>
    </div>
