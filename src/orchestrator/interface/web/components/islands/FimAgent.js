    ...
    if (this.alerts.length === 0) {
      container.innerHTML = `
        <div class="flex flex-col items-center justify-center p-24 opacity-20">
           ...
           <div class="mono-xs font-black text-slate-500 uppercase tracking-[0.4em] animate-pulse">Awaiting_Integrity_Signals...</div>
        </div>
      `;
      return;
    }
        ...
        <div class="p-6 border-b border-white/[0.03] hover:bg-white/[0.02] transition-all animate-fade-in group" 
          ...
          <div class="flex justify-between items-center mb-3">
             <div class="flex items-center gap-4">
                <span class="mono-xs font-black uppercase tracking-widest \${isCritical ? 'text-danger shadow-danger' : 'text-warning shadow-warning'}">
                  \${alert.action || 'MODIFY'}
                </span>
                <span class="dot \${isCritical ? 'danger shadow-danger' : 'warning shadow-warning'}" style="width: 4px; height: 4px;"></span>
          ...
          <div class="flex items-start gap-4">
             <div class="mono-sm font-bold text-slate-400 uppercase tracking-tight break-all leading-relaxed group-hover:text-white transition-colors">
               \${window.escapeHTML(alert.path)}
             </div>
          </div>
          <div class="mt-4 flex gap-4">
             <div class="status-pill \${isCritical ? 'danger' : 'warning'} py-1 px-3 text-[8px] font-black uppercase tracking-widest shadow-inner">UNAUTHORIZED_ACCESS</div>
