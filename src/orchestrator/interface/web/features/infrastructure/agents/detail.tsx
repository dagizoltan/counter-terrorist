...
export const AgentDetailPage = (props: { agent: { name: string; status: string; details?: any; description: string; }, csrfToken?: string, nonce?: string }) => {
  ...
  return (
    <Layout nonce={props.nonce} title={`Agent Detail: ${agent.name}`} islandPaths={islandPaths} csrfToken={props.csrfToken}>
        ...
        <div class="flex items-center gap-6">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="flex items-center gap-4">
              ...
              <div class="mono text-[10px] font-bold text-slate-500 tracking-[0.15em] uppercase">Identity_Verified: SHA-256_ACTIVE</div>
            </div>
          </div>
        </div>
         ...
         {[
           ...
           <div class="t-panel">
              <div class="flex justify-between items-start mb-6">
                 <span class="mono text-[10px] font-black text-slate-500 uppercase tracking-widest">{stat.label}</span>
                 <span class="mono text-[9px] font-black opacity-20 tracking-tighter italic" style={`color:${stat.colorVar}`}>{stat.icon}_BLOCK</span>
              </div>
              <span id={stat.id} class="text-3xl font-black text-white italic tracking-tighter truncate uppercase tabular-nums">WAIT...</span>
           </div>
         ))}
      </div>
      ...
      <div class="grid grid-cols-12 gap-4">
        ...
        <div class="col-span-4 space-y-8">
          <section class="t-panel" style="border-left: 4px solid var(--primary); background: hsla(var(--bg-h), var(--bg-s), 4%, 0.4);">
             <h2 class="tactical-title mb-10 pb-6 border-b border-white/5 flex items-center justify-between" style="font-size:1rem;">
                COMMAND_INTERFACE
                ...
                {agent.name === 'vpn' && (
                  <div class="space-y-6">
                    <div class="p-6 bg-black/40 border border-white/5">
                       <p class="metric-tag mb-6 flex items-center gap-3">
          ...
          <section class="t-panel">
             ...
             <div class="space-y-6">
                <div class="flex justify-between items-center">
                   <span class="mono text-[10px] text-slate-500 uppercase tracking-widest">Integrity_Hash</span>
                   <span class="mono text-[10px] font-black text-success italic tracking-widest uppercase">Verified_Secure</span>
                </div>
                <div class="flex justify-between items-center">
                   <span class="mono text-[10px] text-slate-500 uppercase tracking-widest">Audit_Stability</span>
