...
import { HoneypotService } from "../defense/honeypot_service.ts";
import { ProcessTracker } from "./process_tracker.ts";
import { KernelService } from "../orchestration/kernel_service.ts";
import { AuditService } from "./audit.ts";
import { CanaryService } from "../defense/canary_service.ts";
    ...
    private async collectAndBroadcast() {
        try {
            ...
            const metrics: SystemMetrics = {
                ...
                mesh: {
                    activeNodes: meshNodes.filter(n => Date.now() - n.lastSeen < 60000).length,
                ...
                canary: {
                    ...
                    triggered: canaryTokens.filter(t => t.triggered).length,
                },
                audit: {
