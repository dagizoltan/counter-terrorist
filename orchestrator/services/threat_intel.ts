  ...
  async updateThreatList() {
    ...
    try {
      ...
      const ips = text.split("\n")
        ...
        .slice(0, 50); // Limit to top 50 for demo stability

      this.logging.log(`[INTEL] Synchronized ${ips.length} high-confidence malicious IPs.`, SyslogSeverity.NOTICE);
      
      for (const ip of ips) {
        // Pre-emptively block in the firewall
        await this.protection.firewall.blockIp(ip);
      }
    } catch (e) {
      this.logging.log(`[INTEL] Failed to fetch threat list: ${e.message}. Using local cache.`, SyslogSeverity.WARNING);
    }
  }
